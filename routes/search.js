...
router.get('/:method/:word',function(req, res, next){
  ...
  if(req.params.method === 'tag'){
    sql = 'SELECT `xblog_userinfo`.`uid`,`xblog_userinfo`.`name`,`xblog_userinfo`.`email`,`xblog_userinfo`.`password`,`xblog_userinfo`.`url`,`xblog_userinfo`.`registered`,`cid`,`title`,`content`,`tags`,`categories`,`date`,`status`,`commentstatus`,`priority` FROM `xblog_userinfo` INNER JOIN  `xblog_articles` ON `xblog_userinfo`.`uid` = `xblog_articles`.`uid` WHERE `status` = 1 AND (`xblog_articles`.`tags` REGEXP ?) ORDER BY `cid` DESC';
    sqlParam = ['.*,?'+ req.params.word +',?.*'];
    flag = true;
  }else if(req.params.method === 'category'){
    sql = 'SELECT `xblog_userinfo`.`uid`,`xblog_userinfo`.`name`,`xblog_userinfo`.`email`,`xblog_userinfo`.`password`,`xblog_userinfo`.`url`,`xblog_userinfo`.`registered`,`cid`,`title`,`content`,`tags`,`categories`,`date`,`status`,`commentstatus`,`priority` FROM `xblog_userinfo` INNER JOIN  `xblog_articles` ON `xblog_userinfo`.`uid` = `xblog_articles`.`uid` WHERE `status` = 1 AND (`xblog_articles`.`categories` REGEXP ?) ORDER BY `cid` DESC';
    sqlParam = ['.*,?'+ req.params.word +',?.*'];
    flag = true;
  }else if(req.params.method === 'user'){
    sql = 'SELECT `cid`,`title`,`content`,`tags`,`categories`,`date`,`status`,`commentstatus`,`priority`,? as `name` FROM `xblog_articles` WHERE `uid` = (SELECT `uid` FROM `xblog_userinfo` WHERE `name` = ?) AND `status` = 1 ORDER BY `cid` DESC';
